...
% Mode Humain vs IA
lancerMode(2):- jouerHvsIAFacile([vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide(),vide()],0).
...
jouerHvsH(Plateau):- nl, afficherPlateau(Plateau,0), nl, selectPiece(P,Plateau), informerPieceChoisie(P),afficherPlateau(Plateau,0),nl,placerPiece(P,Plateau,NouveauPlateau), verifVictoire(NouveauPlateau,1,_).

% Cas où c'est l'IA qui sélectionne la pièce, et le joueur qui la place
jouerHvsIAFacile(Plateau,0) :- nl,afficherPlateau(Plateau,0), nl, selectionnerPieceIAFacile(P,Plateau), informerPieceChoisie(P), placerPiece(P,Plateau,NouveauPlateau),verifVictoire(NouveauPlateau,2,0).
% Cas où c'est le joueur qui sélectionne la pièce, et le joueur qui la place
jouerHvsIAFacile(Plateau,1) :- nl,afficherPlateau(Plateau,0), nl, selectPiece(P,Plateau), informerPieceChoisie(P),placerPieceIAFacile(P,Plateau,NouveauPlateau),verifVictoire(NouveauPlateau,2,1).

informerPieceChoisie(P) :- nl, write("La piece choisie est : "),afficherCase(P),nl.
...
placerPiece(P,Plateau,NouveauPlateau):- nl, write("A l'autre joueur de jouer."), nl, nl, repeat, write("Choisissez la ligne"), nl, read(X), X > 0, X < 5, write("Choisissez la colonne"), nl, read(Y), Y > 0, Y < 5, Ind is (X-1)*4+Y, nth1(Ind,Plateau,vide()),changerElemListe(Ind,P,Plateau,NouveauPlateau).
...
verifVictoire(Plateau,_,_):- verifLignes(Plateau), nl, write("Victoire sur la ligne !"),nl,afficherPlateau(Plateau,0).
verifVictoire(Plateau,_,_):- verifColonnes(Plateau), nl, write("Victoire sur la colonne !"),nl,afficherPlateau(Plateau,0).
verifVictoire(Plateau,_,_):- verifDiagonales(Plateau), nl, write("Victoire sur la diagonale !"),nl,afficherPlateau(Plateau,0).

% Vérifie si toutes les pièces ont été sélectionnées
verifVictoire(Plateau,_,_):- listePiecesRestantes(Plateau,[]),nl,write("Fin de partie, égalité, plus de pièces disponibles..."),nl,afficherPlateau(Plateau,0).

%Cas où il n'y a pas de victoire
%Cas où le mode de jeu est Humain contre Humain
verifVictoire(Plateau,1,_):- write("Pas de victoire"),nl,jouerHvsH(Plateau). %dernier cas, il n'y a pas de victoire

% Cas où le mode de jeu est Humain contre IA :

% Cas où l'humain vient de placer, c'est au tour de l'IA de placer
verifVictoire(Plateau,2,0):- write("Pas de victoire"),nl,jouerHvsIAFacile(Plateau,1). 
% Cas où l'IA vient de placer, c'est au tour de l'humain de placer 
verifVictoire(Plateau,2,1):- write("Pas de victoire"),nl,jouerHvsIAFacile(Plateau,0). 
